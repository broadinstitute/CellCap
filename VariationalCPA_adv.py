...
#         )
#
#         self.d_encoder = DrugEncoder(n_latent,n_drug)
#         self.donor_encoder = DrugEncoder(n_latent,n_donor)
#
...
#
#         input_dict = dict(
#             x=x, batch_index=batch_index, pert_index=pert_index, d=d, donor=donor, cont_covs=cont_covs, cat_covs=cat_covs
#         )
#         return input_dict
#
...
#
#     @auto_move_data
#     def inference(self, x, d, donor, batch_index, pert_index, cont_covs=None, cat_covs=None, n_samples=1):
#         """
...
#         outputs = dict(z=z, qz_m=qz_m, qz_v=qz_v, ql_m=ql_m, ql_v=ql_v, prob=prob, Zp=Zp, Zd=Zd, library=library)
#         return outputs
#
#
#     @auto_move_data
#     def generative(self,z,Zp,Zd,library,batch_index,cont_covs=None,cat_covs=None,size_factor=None,y=None,transform_batch=None):#
...
#
#         #loss = torch.mean(reconst_loss*10.0 + weighted_kl_local + advers_loss*5.0 + ent_penalty*2.0)
#         #loss = torch.mean(reconst_loss + weighted_kl_local*0.8 + advers_loss*0.5) + ent_penalty*0.3 + cos_penalty*0.5
#         #loss = torch.mean(reconst_loss*0.5 + weighted_kl_local*0.5 + advers_loss) + ent_penalty*0.2 + cos_penalty
#         #loss = torch.mean(reconst_loss*0.5 + weighted_kl_local + advers_loss + ent_penalty*0.5)
#         #loss = torch.mean(reconst_loss*0.5 + weighted_kl_local + advers_loss + ent_penalty*0.5)
#         #loss = torch.mean(reconst_loss*0.5 + weighted_kl_local + ent_penalty*0.5) + advers_loss*5.0
#         #loss = torch.mean(reconst_loss*20.0 + weighted_kl_local + ent_penalty  + advers_loss)
#         loss = torch.mean(reconst_loss + weighted_kl_local + ent_penalty + advers_loss)
#
#         kl_local = dict(
