...
#         self.apply(init_weights)
#         self.encoder = torch.nn.Linear(self.input_dim,self.output_dim)
#         #self.encoder = torch.nn.Sequential(
#         #    torch.nn.Linear(self.input_dim,10),
...
#         self.state_encoder = StateEncoder(n_latent,self.n_drug)
#
...
#
#         sz = self.state_encoder(z)
#         sz = d * sz
#         Zp = self.d_encoder(sz)
#
#         sz = torch.sum(sz, dim=1)
#         sz = torch.unsqueeze(sz, 1)
#
...
#         outputs = dict(z=z, qz_m=qz_m, qz_v=qz_v, ql_m=ql_m, ql_v=ql_v, prob=prob, Zp=Zp, Zd=Zd, library=library, sz=sz)
#         return outputs
#
#
#     @auto_move_data
...
#
#         #loss = torch.mean(reconst_loss*5.0 + weighted_kl_local*10.0 + ent_penalty*2.0 + advers_loss*10.0)
#         #loss = torch.mean(reconst_loss + weighted_kl_local + ent_penalty*0.25 + advers_loss)
#         loss = torch.mean(reconst_loss*0.5 + weighted_kl_local + ent_penalty*0.25 + advers_loss)
#         #loss = torch.mean(reconst_loss*0.25 + weighted_kl_local*0.5 + ent_penalty*0.1 + advers_loss)
#
#         kl_local = dict(
