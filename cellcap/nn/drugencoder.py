    ...
    def __init__(self, latent_dim=20, drug_dim=2, prog_dim=10, key=True):
        ...
        if key:
            self.weight = torch.nn.Parameter(
                torch.rand(self.drug_dim, self.n_prog, self.latent_dim)
            )
        else:
            self.weight = torch.nn.Parameter(
                torch.zeros(self.drug_dim, self.n_prog, self.latent_dim)
            )

    def forward(self, x):
        y = torch.matmul(
            x, self.weight.reshape((self.drug_dim, self.n_prog * self.latent_dim))
        )
...
class DrugEncoder(torch.nn.Module):
    def __init__(self, latent_dim=20, drug_dim=2, prog_dim=10, key=True):
        ...
        self.drug_weights = MarkerWeight(
